"""PEMS08 config — CLFMv2 Ablation: No Smoothness Loss."""
import os, sys
...
from idea.CLFM.v2.ablations import CLFMv2_NoSmoothnessLoss
...
MODEL_ARCH = CLFMv2_NoSmoothnessLoss
MODEL_PARAM = {
    "num_nodes": 170, "input_len": INPUT_LEN, "input_dim": 3,
    "output_len": OUTPUT_LEN,
    "field_dim": 32, "hidden_dim": 64, "num_pde_steps": 4,
    "encoder_layers": 2, "decoder_layers": 2, "pde_layers": 2,
    # smoothness_weight not used by this model but kept for API compat
...
def _mae_only_loss(prediction, target, null_val=0.0, **kwargs):
    """Loss for No-Smoothness ablation: pure MAE, no auxiliary terms."""
    return masked_mae(prediction, target, null_val)


CFG = EasyDict()
CFG.DESCRIPTION = 'CLFMv2 Ablation: No Smoothness Loss on PEMS08'
CFG.GPU_NUM = 1
