"""
CLFMv2 Ablation: No Continuous State Space (SSM)
=================================================
Removes the SSM temporal refinement after each PDE evolution step.
The PDE step becomes a raw Euler update without state-space filtering.

Purpose: Measure whether the SSM's temporal memory and skip connection
         contribute beyond the basic Euler PDE evolution.
"""
...
class CLFMv2_NoSSM(nn.Module):
    """CLFMv2 without the Continuous State Space model.

    PDE step: dF/dt = α·L·F + (1-α)·N_θ(F), Euler update only (no SSM).
    """
    ...
    def __init__(self, **model_args):
        ...
        # NO ContinuousStateSpace — ABLATED
    ...
    # ------------------------------------------------------------------
    def _pde_step(self, field, dt=1.0):
        """PDE step WITHOUT SSM: pure Euler update, no temporal refinement."""
        ...
        # Raw Euler step — NO SSM refinement
        field_evolved = field + dF * dt

        return field_evolved, dF
    ...
    def forward(self, history_data, future_data, batch_seen, epoch, train, **kwargs):
        ...
        for _ in range(self.num_pde_steps):
            field, dF = self._pde_step(field, dt)
