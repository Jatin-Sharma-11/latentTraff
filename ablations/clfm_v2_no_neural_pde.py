"""
CLFMv2 Ablation: No Neural PDE Operator
========================================
Removes the neural nonlinear term  (1-α)·N_θ(F)  from the PDE.
The PDE becomes:  dF/dt = L_road · F   (pure linear diffusion).

Purpose: Measure whether the nonlinear neural operator contributes
         beyond what the linear Laplacian diffusion already captures.
"""
...
class CLFMv2_NoNeuralPDE(nn.Module):
    """CLFMv2 without the Neural PDE Operator.

    PDE step becomes:  dF/dt = σ(w)·L_road·F
    (pure linear diffusion, no nonlinear N_θ term, no pde_mix α).
    """
    ...
    def __init__(self, **model_args):
        ...
        # NO Neural PDE Operator — ABLATED

        # Continuous State Space (kept)
        ...
        # No pde_mix needed — purely linear diffusion
    ...
    def _pde_step(self, field, state, dt=1.0):
        """PDE step WITHOUT Neural PDE: dF/dt = L_road · F (pure diffusion)."""
        dF = self.laplacian_op(field)
